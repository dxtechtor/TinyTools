...
# def register():
    ...
    # try:
        # with connect_db() as conn:
            # # UNIQUE CHECK: Check if email (case-insensitive) or phone already exists
            # existing = conn.execute(
                # "SELECT userid FROM users WHERE userid=? COLLATE NOCASE OR phone=?", 
                # (email, phone)
            # ).fetchone()
            # if existing:
                # return jsonify({
                    # "error": "The email or phone already exists. Please login or contact 9024676486 to reset password"
                # }), 400
...
# def check_update():
    # return jsonify({
        # "version_code": 5, 
        # "server_url": "http://10.218.57.59:5050/api/", 
...
def admin_users_list():
    if 'pending_users' in request.path: 
        status_filter = "status='pending'"
        select_cols = "*" # Pending needs images for verification
    elif 'deleted_users' in request.path: 
        status_filter = "status='deleted'"
        select_cols = "userid, name, email, shop_name, role, score, expiry_date, referral_code as uid"
    else: 
        status_filter = "status!='deleted' AND status!='pending'"
        # Optimized: Exclude images from the general list to save data
        select_cols = "userid, name, email, shop_name, role, shop_type, phone, city, status, score, expiry_date, referral_code as uid, reference_name"
    
    with connect_db() as conn:
        return jsonify([dict(r) for r in conn.execute(f"SELECT {select_cols} FROM users WHERE {status_filter}").fetchall()])

@app.route('/api/admin/get_user_images', methods=['GET'])
def get_user_images():
    uid = request.args.get('userid')
    with connect_db() as conn:
        # We add 'COLLATE NOCASE' to make the search ignore Capital/Small letters
        user = conn.execute("SELECT profile_img, license_img FROM users WHERE userid=? COLLATE NOCASE", (uid,)).fetchone()
        if user:
            ...
            if req:
                ...
                if req['req_action'] == 'remove':
                    ...
                    for s in json.loads(req['stokist_list']): 
                        ...
                        # NEW: Add to stokist_master so it shows up in global dropdowns
                        if " (" in s and s.endswith(")"):
                            name_part = s[:s.rfind(" (")].strip()
                            area_part = s[s.rfind(" (")+2:-1].strip()
                            conn.execute("INSERT OR IGNORE INTO stokist_master (name, area) VALUES (?,?)", (name_part, area_part))
                        else:
                            conn.execute("INSERT OR IGNORE INTO stokist_master (name, area) VALUES (?,?)", (s, "General"))
                
                conn.execute("UPDATE requests SET status='approved', action_at=? WHERE id=?", (now, rid))
                # Reward 5 points per approved contribution
                add_score(req['userid'], 5, f"Contribution Approved: {req['name']}")
    return jsonify({"status": "ok"})
